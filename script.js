            ...
            appData.forEach(row => {
                ...
                const renderInput = (field, size, isLast) => `
                    <td class="p-1 ${isLast ? 'border-r-2 border-slate-400 dark:border-slate-500' : 'border-r border-slate-200 dark:border-slate-700'}">
                        <input type="number" min="0" step="any" value="${row[field][size]}" onkeydown="if(['-','+','e','E'].includes(event.key))event.preventDefault()" oninput="if(this.value<0)this.value=Math.abs(this.value); updateData('${row.id}', '${field}', '${size}', this.value)" 
                ...
                tr.innerHTML = `
                    ...
                    <td class="p-1 border-r border-slate-200 dark:border-slate-700 bg-red-50/20 dark:bg-red-900/10">
                        <div class="relative w-full flex items-center">
                            <input type="number" min="0" step="any" value="${row.extraDiscount || ''}" placeholder="0" onkeydown="if(['-','+','e','E'].includes(event.key))event.preventDefault()" oninput="if(this.value<0)this.value=Math.abs(this.value); updateData('${row.id}', 'extraDiscount', null, this.value)" 
